...
\caption{Same, multi-day carry: $\bar I^{\text{ref}}$ and inventory persist
         across sessions with overnight decay. Net P\&L (blue) exceeds
         gross (orange) because the Waelbroeck mark-to-market credits
         the carried long position for its own simulated price
         displacement -- an accounting artefact discussed in the
         conclusion.}
\end{figure}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Enhancement 2 --- multi-day carry backtest
        ...
        sessions \emph{quadruples} gross P\&L ($\$118$k$\to\$428$k) as
        the strategy accumulates long inventory through the trend.
        Daily-reset wins on Sharpe (+3.43 vs +2.05) with tight
        drawdowns; multi-day wins on absolute dollars but introduces a
        subtle artefact: the Waelbroeck mark-to-market credits the
        carried long position for its own price displacement,
        producing a negative reported impact cost ($-\$39$k).  This is
        an accounting effect, not free money; a clean impact-cost
        decomposition for persistent inventory needs implementation-
        shortfall against an arrival-price benchmark, which we did
        not implement here.  The carry flag is a single
        \texttt{BacktestConfig} field; downstream code is unchanged.
