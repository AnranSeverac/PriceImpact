...
\begin{frame}{Sensitivity \& stress \hfill \emph{\S2.7}}{Live backtest sweeps + worked stress scenario}
  ...
  Wrong model (OW $\to$ AFS)     & \warn{$-$3.48} & \warn{$-$\$863k} \\
  ...
  \textbf{Takeaways}
  \begin{itemize}
    \item \warn{Wrong model catastrophic} -- $-\$863$k
    \item \warn{1-min delay flips the sign} -- $\alpha$ is short-horizon
    \item $\rho$ scales Sharpe linearly: $\{0.01, 0.05, 0.10\}\!\to\!\{0.45, 4.30, 9.22\}$
    \item $H$ is forgiving: $\pm 2\times$ stays in $[+3.3, +5.9]$
  \end{itemize}
  ...
  \vspace{0.1em}
  \footnotesize\itshape
  Gross alpha-capture (orange dashed) still builds steadily -- the signal is real. But OW-sized positions under AFS impact pay \warn{${\sim}\$995$k} in impact cost, dwarfing gross P\&L. Net (blue) collapses early.
