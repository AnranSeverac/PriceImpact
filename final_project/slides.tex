...
\begin{frame}{Impact-state recursion \hfill \emph{\S2.2}}{OW vs AFS, daily-reset vs multi-day $\bar I$}
  ...
  \vspace{-0.2em}
  \scriptsize\itshape
  The sqrt form is chosen so the model reproduces the empirical
  meta-order law $\Delta P / P_0 \propto \sqrt{Q/V}$
  (Almgren et al.; Bouchaud): a constant-rate parent of size $Q$
  builds equilibrium $\bar I^* = f(Q/T)/\beta$, whose shape inherits
  $f$. OW would predict linear-in-$Q$ impact, which the data rejects.
...
\begin{frame}{Half-life grid search \hfill \emph{\S2.2}}{Empirical $H$ via rolling-OOS sweep}
  ...
  \footnotesize
  \textbf{Optimisation problem.} Joint $\max_{H,\alpha,\lambda} R^2_{\text{OOS}}(H,\alpha,\lambda)$.
  For fixed $H$, OLS pins $(\hat\alpha,\hat\lambda)$ in closed form, so the problem reduces to a 1-D scan over $H$ -- the half-life that yields the best-fitting impact regression:
  \[
    \max_{H}\; R^2_{\text{OOS}}\!\bigl(H,\,\hat\alpha(H),\,\hat\lambda(H)\bigr).
  \]
  Grid: $H \in \{5,10,20,30,45,60,90,120,180\}$ min.
  ...
  \begin{itemize}
    \item Per-model optima differ: OW peaks at 60, AFS at 45
    \item Surface \hilite{flat in 45--60 min}: $<10^{-4}$ $R^2$ cost anywhere in band
    \item Midpoint is model-agnostic; lets OW + AFS share bins + comparisons
  \end{itemize}
  \end{columns}
\end{frame}
...
% ============================================================
\begin{frame}{Non-parametric extension + $\gamma$ shrinkage \hfill \warn{ENHANCEMENT \#1}}{Method + regime-correctness stress}
  ...
  Tuning $\gamma$ on the validation month leaks. We need \emph{train, tune, validate} -- distinct months, in order.
  ...
  \column{0.56\textwidth}
  \footnotesize
  \textbf{Regime-correctness stress.} Headline ($n \approx 3000$/cell): tuning curves are flat $\Rightarrow$ shrinkage has nothing to fix. Stress: shrink train window 1\,mo $\to$ 5\,d $\to$ 2\,d so per-cell $n$ drops 10$\times$.

  \vspace{0.2em}
  \centering
  \includegraphics[width=0.98\linewidth]{figures/stress6b_tuning_progression.png}

  \vspace{0.2em}
  \begin{tabular}{lccc}
  \toprule
  Train & Med.\,$n$ & OW gain (reg$-$raw) & $\gamma^*/n$ \\
  \midrule
  ...
  \vspace{0.2em}
  \footnotesize\itshape
  OOS gain grows ${\sim}14\times$ as data shrinks ${\sim}10\times$; shrinkage strength rises 6$\times$. The estimator is \hilite{regime-correct}: trusts per-stock data when there is plenty, pools toward universal when starved.
  \end{columns}
\end{frame}
